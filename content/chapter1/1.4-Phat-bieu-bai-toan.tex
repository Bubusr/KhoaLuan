...
Từ câu hỏi, bối cảnh bệnh nhân và lịch sử hội thoại, hàm tiền xử lý $f_{\mathrm{pre}}$ tạo truy vấn độc lập
\[
    q'=f_{\mathrm{pre}}(q,u,h).
\]
Quá trình này bao gồm kiểm tra đầu vào, cô đọng lịch sử, xác định ý định và trích xuất thực thể y khoa. Vector truy vấn được tạo bởi mô hình nhúng $E_{\phi}$:
\[
...
\]
trong đó $s_{\mathrm{dense},\phi}$ là điểm truy xuất đặc dựa trên biểu diễn ngữ nghĩa, $s_{\mathrm{sparse}}$ là điểm truy xuất thưa dựa trên từ khóa, và $\alpha\in[0,1]$ là hệ số phối hợp hai thành phần. Tập bằng chứng được chọn là
\[
    C^*=\operatorname{TopK}_{c\in C}\,s_{\phi}(q',c),
\]
sau đó có thể được lọc theo siêu dữ liệu và xếp hạng lại.

Gọi $\bar{h}$ là phần lịch sử hội thoại đã được cô đọng và $I_{\mathrm{safe}}$ là chỉ dẫn an toàn của hệ thống. Câu lệnh cuối cùng $P$ được xây dựng bởi
\[
    P=\operatorname{Concat}(I_{\mathrm{safe}},q',u,\bar{h},C^*).
\]
LLM với tham số $\theta$ sinh câu trả lời nháp theo
\[
    \widetilde{y}
    =\arg\max_{y}P_{\theta}(y\mid q',u,\bar{h},C^*).
\]
Lớp bảo đảm chất lượng $g_{\psi}$ đánh giá truy vấn, bằng chứng và câu trả lời nháp để tạo vector điểm $\mathbf{v}$ và quyết định
\[
    (\mathbf{v},d)=g_{\psi}(q',u,C^*,\widetilde{y}),
\]
với chính sách tổng quát
\[
    d=
    \begin{cases}
        \text{Answer}, & \text{nếu bằng chứng đầy đủ, câu trả lời bám sát nguồn và an toàn},\\
        \text{Abstain}, & \text{nếu bằng chứng thiếu, mâu thuẫn hoặc độ tin cậy thấp},\\
        \text{Escalate}, & \text{nếu phát hiện tình huống nguy cơ cao hoặc cần thăm khám trực tiếp}.
    \end{cases}
\]
